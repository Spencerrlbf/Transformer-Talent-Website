...
const config={run,dry:false,concurrency:1};
try{
 ...
 assert.equal((await db.query('select count(*)::int n from candidate_profile_state')).rows[0].n,1);
 ...
 console.log('PASS actual local runner: interruption after commit, restart, full coverage, idempotency, live candidate equality, bounds');
