...
function projectionInput(id, t) {
  ...
  return {
    candidate_id: id,
    header,
    header_provenance: state?.header ?? null,
    jobs_source: (t.sources.get(id) ?? []).find((s) => s.id === state?.jobs_source_id) ?? null,
    state,
...
export function allowedParserReplay(stored, doc, enabled) {
  ...
  return !!enabled && stored.parser_version === "person-v2" && incoming.parser_version === "person-v3"
    && stored.source === incoming.source && String(stored.source_ref ?? "") === String(incoming.source_ref ?? "")
    && Date.parse(stored.fetched_at) === Date.parse(incoming.fetched_at);
}
...
async function main() {
  ...
  const ALLOW_PARSER_REPLAY = truthy(process.env.ALLOW_PARSER_REPLAY);
