...
export function fromDirectory(
  ...
  const header = makeHeader({
    current_title: clean(boardRow.title) ?? clean(h?.current_title),
    current_company: clean(boardRow.company) ?? clean(h?.current_company),
    full_name: fullName,
  ...
  const boardAt = isoOf(boardRow.updated_at);

  return assembleDoc({
    candidate_id: candidateId,
    // An absent list is not asserted; header-only directory facts still apply.
    mode: "replace_lists",
    source: {
      source: "directory",
      provider: "comms",
    ...
    identities,
    header,
    header_dates: boardAt ? {
      ...(clean(boardRow.title) ? { current_title: boardAt } : {}),
      ...(clean(boardRow.company) ? { current_company: boardAt } : {}),
    } : undefined,
