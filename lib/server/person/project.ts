...
import { cleanText } from "../spine";
import { isClearSideRoleTitle, realJobIndex } from "./role-selection";
...
export function project(tables: ProjectionInput): Projection {
  ...
  const currentFact = (key: "current_title" | "current_company", fallback: string | null): string | null => {
    ...
    const fact = tables.header_provenance?.[key];
    if (!fallback || fact?.source === "recruiter") return value;
    const factAt = fact ? Date.parse(fact.at) : NaN;
    const jobsAt = tables.jobs_source ? Date.parse(tables.jobs_source.fetched_at) : NaN;
    return Number.isFinite(factAt) && Number.isFinite(jobsAt) && factAt >= jobsAt ? value : fallback;
  };
  return {
    current_title: currentFact("current_title", current ? txt(current.title) : null),
    current_company: currentFact("current_company", current ? companyName(current) : null),
    work_experience,
