...
import { isClearSideRoleTitle, isSideRoleTitle, realJobIndex } from "./role-selection";
...
export const PARSER_VERSION = "person-v3";
...
export function makeHeader(h: Partial<Record<keyof PersonHeader, unknown>>): PersonHeader {
  return {
    current_title: clean(h.current_title),
    current_company: clean(h.current_company),
...
export function assembleDoc(args: {
  ...
  header: PersonHeader;
  header_dates?: PersonDoc["header_dates"];
  jobs?: PersonJob[];
  ...
  const content: Omit<PersonDoc, "source"> = {
    ...
    identities,
    header: args.header,
    ...(args.header_dates ? { header_dates: args.header_dates } : {}),
    jobs: list(args.jobs),
  ...
  if (content.header_dates) doc.header_dates = content.header_dates;
