...
export function harvestToPoolRecord(harvest: Record<string, unknown> | null): Record<string, unknown> {
  ...
  const side = (p: { title: string | null; company: string | null }) => isSideRoleTitle(p.title, p.company);
  const job = realJobIndex(positions, side, (p) => isClearSideRoleTitle(p.title, p.company));
